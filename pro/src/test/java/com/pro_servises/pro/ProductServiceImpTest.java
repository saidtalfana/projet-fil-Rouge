...
public class ProductServiceImpTest {
    ...
    void testAddProductDto() {
        ...
        // When
        ProductDto result = productServiceImp.addProductDto(productDto, 1, null);
    ...
    void testAddProductDto_Conflict() {
        ...
        // Mocking the repository to simulate that the product already exists
        when(productRepository.findByName(productDto.getName())).thenReturn(new Product());

        // Mocking the enterprise repository to return a valid enterprise
        when(entepriseRepository.findById(anyInt())).thenReturn(Optional.of(new Enterprise()));

        // When/Then
        ConflictException thrown = assertThrows(
                ConflictException.class,
                () -> productServiceImp.addProductDto(productDto, 1, null) // Assuming imageBytes is null for this test
        );
